...
/**
 * Dynamically overlays a watermark onto an image buffer.
 * The watermark text contains the Club (Category) name, Event name, and User role.
 * @param {Buffer} fileBuffer Original image buffer
 * @param {Object} details Watermark configuration details
 * @param {string} details.clubName Name of the club or category
 * @param {string} details.eventName Name of the event
 * @param {string} details.userRole Role of the downloading user
...
export const applyWatermark = async (fileBuffer, { clubName = 'Club', eventName = 'Event', userRole = 'Viewer' } = {}) => {
  try {
    ...
    // Create an SVG overlay that fits the image size
    // We will place a diagonal semi-transparent watermark text across the image
    // AND a solid branding strip at the bottom.
    const svgOverlay = `
    ...
    // Composite the SVG watermark on top of the original image
